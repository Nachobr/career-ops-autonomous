...
export function roleFuzzyMatch(a, b) {
  ...
  // Identical content-token sets (with at least two shared tokens) are always
  // the same opening. This short-circuit is required because some titles are
  // composed entirely of baseline vocabulary (e.g. "Forward Deployed Software
  // Engineer" → forward+deployed+software+engineer are all baseline). Without
  // it the discriminating-token check below would reject two identical such
  // titles and let a real duplicate slip into the tracker.
